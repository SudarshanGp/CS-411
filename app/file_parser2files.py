...
def buildOutputSQL(file_name, db_name):
   ...
   f.write("DROP TABLE IF EXISTS "+db_name+".Rank;\n")
   f.write("CREATE TABLE "+db_name+'''.Rank (
    ...
    Junior int not null,
    Senior int not null,
    Non-degree Ugrad int not null,
    Masters int not null,
    Doctoral int not null,
    primary key(ID),
    FOREIGN KEY(ID) REFERENCES '''+ db_name+'''.id(ID) 
    ON DELETE CASCADE
    ON UPDATE CASCADE
...
def FilePar2(fileloc, datb):
			...
			if(current_sheet.cell(i,2).value=='  ' and current_sheet.cell(i,3).value=='  ' and 
				...
				if(tempc in Gender.keys()):
                    ...
                    if(type(current_sheet.cell(i,11).value) is float):
						Rank[j][0]+=current_sheet.cell(i,11).value
					if(type(current_sheet.cell(i,12).value) is float):
						Rank[j][1]+=current_sheet.cell(i,12).value
					
                   
                    if(type(current_sheet.cell(i,14).value) is float):
						Rank[j][0]+=current_sheet.cell(i,14).value
					if(type(current_sheet.cell(i,15).value) is float):
						Rank[j][1]+=current_sheet.cell(i,15).value
    ...
    return Department,Rank, retCounter   
...
def main():
    ...
    if sys.argv[3]== 'Rank':
        filetype='Rank'
    else if sys.argv[3]== 'GER':
        filetype='GER'
    else   
        print 'Enter a file type'
    
    if(len(sys.argv) == 4):
        createNew = str(sys.argv[3])
    ...
    if sys.argv[3]=='GER':
    ...
    else if sys.argv[3]=='Rank':
        Department2, Rank=FilePar2(fileloc,dbName)
        ids = []
        Ranklist=[]
      
        for key in Department2:
            for val in Department2[key]:
                temp=[]
                temp.append(str(Department2[key][val]))
                temp.append('"'+str(sys.argv[1])[:-4]+'"')
                temp.append(key)
                temp.append(val)
                ids.append(temp)
        ...
        for key in Rank:
            ...
            temp.append(Rank[key][5])
            temp.append(Rank[key][6])
        ...
        #writeDel(rm_file, dbName, "Rank", first)
